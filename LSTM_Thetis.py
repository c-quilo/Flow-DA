...
np.save(print_directory_model + 'reduced_pcs', pcs)
np.save(print_directory_model + 'reduced_eofs', eofs)

meanThetis = np.mean(modelThetis2D, axis = 0)
np.save(print_directory_model, meanThetis)
...
model = keras.models.Sequential([
    keras.layers.LSTM(units = 22, input_shape = (X_train.shape[1], X_train.shape[2]), return_sequences = True),
