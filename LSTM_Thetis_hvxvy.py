"""
Long-short term momery neural network replicating the physics learned from Thetis
"""
import numpy as np
import sys, os
...
#Mean to be substracted
meanVx = np.mean(modelThetis2Dvx, axis = 0)
meanVy = np.mean(modelThetis2Dvy, axis = 0)
meanElev = np.mean(modelThetis2Delev, axis = 0)
#Mean acroos all three variables
mean_hvxvy = np.hstack((meanElev, meanVx, meanVy))

#Normalise by standard deviation
sigmaVx = np.std(modelThetis2Dvx)
sigmaVy = np.std(modelThetis2Dvy)
sigmaElev = np.std(modelThetis2Delev)
#Normalised by their standard deviation

np.save(print_directory_model + 'meanThetis_hvxvy', mean_hvxvy)

Nvx = (modelThetis2Dvx - meanVx)/sigmaVx
Nvy = (modelThetis2Dvy - meanVy)/sigmaVy
Nelev = (modelThetis2Delev - meanElev)/sigmaElev

np.save(print_directory_model + 'sigmaVx', sigmaVx)
np.save(print_directory_model + 'sigmaVy', sigmaVy)
np.save(print_directory_model + 'sigmaElev', sigmaElev)

modelData = np.hstack((Nelev, Nvx, Nvy))
...
pcs = solver.pcs(npcs = np.where(varianceCumulative > targetVariance)[0][0], pcscaling = 0)
eofs = solver.eofs(neofs = np.where(varianceCumulative > targetVariance)[0][0], eofscaling = 0)
np.save(print_directory_model + 'reduced_hvxvy_pcs', pcs)
np.save(print_directory_model + 'reduced_hvxvy_eofs', eofs)
...
X = scalerThetis(X, xmin, xmax, min, max)
y = scalerthetis(y, xmin, xmax, min, max)
...
model = keras.models.Sequential([
    keras.layers.LSTM(units = 28, input_shape = (X_train.shape[1], X_train.shape[2]), return_sequences = True),
    keras.layers.Dropout(0.1),
#model.add(LSTM(50, return_sequences = True))
    keras.layers.LSTM(300, return_sequences = False),
    keras.layers.Dropout(0.1),
...
model.fit(X_train, y_train, epochs=1000, batch_size=128, verbose=2, validation_data = (X_test, y_test), shuffle = True)#, callbacks = [tensorboard])
...
# Time-series in the physical space
...
model.save(print_directory_model + 'lstmThetis_hvxvy.h5')
