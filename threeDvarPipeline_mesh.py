...
read_directory_vtu_numpy(directory_model, print_directory_model, field_name)
read_directory_vtu_numpy(directory_obs, print_directory_obs, field_name)

#Covariance

file_name = 'vm_field.npy'
modelThetis2D = np.load(print_directory_model+file_name)
...
np.save(print_directory_model + 'reducedCV', reducedCovariance)
...
start = time.time()
ug = modelThetis2D[1, :]
v0, n, xB, d, uvwVec = PrepareForDA(ug, V)
xDA = ThreeD_VAR(V, v0, n, xB, d)
print(time.time() - start)

#Save to vtu
filename = 'Velocity2d_200.vtu'

ugg = vtktools.vtu(directory_model + filename)
ugg.AddScalarField('DA', xDA)
ugg.AddScalarField('Obs', synthObs)
